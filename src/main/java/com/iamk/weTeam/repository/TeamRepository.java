...
public interface TeamRepository extends JpaRepository<Team, Integer> {
    ...
    @Query(value = "select t.id,t.name,t.contact,t.team_no,t.size,t.brief,g.game_name from team t, game g where t.game_id=g.id and t.user_id=:id order by t.create_time desc", nativeQuery = true)
    ...
    @Query(value = "select t.id,t.name,t.contact,t.team_no,t.size,t.brief,g.game_name,tu.type from team t, game g, team_user tu where t.game_id=g.id and t.id=tu.team_id and t.user_id!=:id and tu.user_id=:id order by t.create_time desc", nativeQuery = true)
    List<Map<String, Object>> findParticipateById(@Param("id")Integer id);

    Team findByNameAndContact(String s, String s1);
}
