    ...
    public ResultUtil wxLogin(@RequestParam("code") String code,
        ...
        // try {
            ...
            if(user == null) {
                ...
                redisUtil.set(key, token, 60*60*24);
            }else {
                // 用户存在
                ...
                userRepository.save(user);
                // 更新redis有效期
                String key = RedisKeyUtil.createKey("user", "userId", user.getId());
                if(redisUtil.hasKey(key)){
                    ...
                    redisUtil.expire(key, 60*60*24*2);
                }else {
                    token = JwtUtils.createJWT(Integer.toString(user.getId()), openId);
                    redisUtil.set(key, token, 60*60*24*2);
                }
            }
            // 封装响应消息
            JSONObject jsonObject = new JSONObject();
