...
public class DateUtil {
    ...
    /**
     * 两个Date相差的天数
     * @param  beforeDate [description]
     * @param  afterDate  [description]
     * @return            [description]
     */
    public static int betweenDays(Date beforeDate, Date afterDate) {
        ...
        if (beforeDate.after(afterDate)) {
            return (int) ((beforeDate.getTime() - afterDate.getTime()) / (1000 * 3600 * 24));
        }
     ...
     * @return  String
     */
    public static String getAddDayTime(Date date, int days){
        return sdfTime.format(new Date(date.getTime()+days*24*60*60*1000));
    }
     ...
     * @return Date
     */
    public static Date getAddMinDate(Date date, int min){
     ...
     * @return
     */
    public static Date getBeforeYear(int n) {
        Calendar calendar = Calendar.getInstance();
